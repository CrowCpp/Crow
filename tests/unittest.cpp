...
TEST_CASE("simple_response_routing_params")
{
  ...
  CHECK("text/xml" == response(100, "xml", "").get_header("Content-Type"));
  CHECK(200 == response(200, "html", "").code);
  CHECK("text/html" == response(200, "html", "").get_header("Content-Type"));
  CHECK(500 == response(500, "html", "Internal Error?").code);
  CHECK("text/css" == response(500, "css", "Internal Error?").get_header("Content-Type"));
